...
import { SolanaAdapter, EthereumAdapter, BalanceAggregator } from '@proofoflove/chain-adapters';
import { WealthProver, WealthVerifier, generateNullifier, getTierBadge, formatBalance } from '@proofoflove/core';
...
async function main() {
  ...
  let config: any;
  try {
    config = JSON.parse(await fs.readFile(path.join(__dirname, 'config.json'), 'utf-8'));
  } catch (error) {
    console.error('✗ Failed to load config.json');
    console.error('Please copy config.example.json to config.json and add your API keys\n');
    process.exit(1);
  }
  ...
  console.log(`✓ Registered chains: ${aggregator.getRegisteredChains().join(', ')}\n`);

  // Step 3: Prepare wallet list
  const wallets: Array<{ chain: string; address: string }> = [
    ...config.testWallets.solana.map((addr: string) => ({ chain: 'solana', address: addr })),
    ...config.testWallets.ethereum.map((addr: string) => ({ chain: 'ethereum', address: addr }))
  ];
  ...
  snapshots.forEach((s, i) => {
    console.log(`  ${i + 1}. ${s.date.toLocaleDateString()} (${s.unixTimestamp})`);
  });
  console.log('');

  // Step 5: Fetch and aggregate balances
  console.log('Fetching historical balances (this may take 30-60 seconds)...\n');
  ...
  const minBalance = Math.min(...aggregatedBalances);
  console.log(`\nMinimum balance: ${formatBalance(minBalance)}\n`);
  ...
  const circuitPath = path.join(__dirname, '../../../circuits/build');
  const prover = new WealthProver(circuitPath);
  ...
  const vkeyPath = path.join(circuitPath, 'keys/verification_key.json');
  let verifier;
  try {
  ...
  console.log('3. ✓ Generated ZK proof WITHOUT revealing exact balances');
